...
__global__ void EmbossGPUKernel(unsigned char* imaged, unsigned char* outputImaged,int width,int height,double* filter){
...
__global__ void EmbossGPUKernel1(unsigned char* imaged, unsigned char* outputImaged,int width,int height){
...
__global__ void EmbossGPUKernel3(unsigned char* outputImaged,int width,int height,double* filter){
...
bool  EmbossGPU( Bitmap* image, Bitmap* outputImage, int choice ){
	...
	if(choice == 1)
	EmbossGPUKernel<<<dimGrid, dimBlock>>>(image_d,Outputimage_d,width,height,filter_d); // Kernel call
	else if(choice == 2)
	EmbossGPUKernel1<<<dimGrid, dimBlock>>>(image_d,Outputimage_d,width,height);
	else if (choice == 3)
	EmbossGPUKernel3<<<dimGrid, dimBlock>>>(Outputimage_d,width,height,filter_d); // Kernel call
	status = cudaGetLastError();
		if (status != cudaSuccess) {
